...
const DetetiveDeProblemas = () => {
  ...
  const generateProblemsResponse = async (input) => {
    const response = await sendMessageToChatbot(`
      A partir de agora, você irá receber uma descrição de uma imagem e um possível problema que o jogador pode ter encontrado no cenário dessa imagem. Sua tarefa é avaliar esse problema de forma objetiva e construtiva. Avalie a criatividade do problema identificado. Você deve analisar se o problema é inovador, interessante e se foge do óbvio. A criatividade deve ser avaliada em uma escala de 0 a 10 (não seja excessivamente criterioso, o problema não precisa ser o menos obvio). Avalie a coerência do problema com a descrição da imagem. O problema faz sentido com o que é descrito na imagem? Está bem fundamentado na descrição visual? A coerência também deve ser avaliada em uma escala de 0 a 10. Lembre se também que o usuário não deve dar uma solução e sim um problema então avalie sobre o problema.
